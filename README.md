...
| [banini-tracker](banini-tracker/) | Track Taiwan's most famous "reverse indicator" investor on Threads, let Claude do the contrarian analysis on the spot. Zero API cost — Playwright scrapes locally, Claude IS the LLM. Rewritten from [cablate/banini-tracker](https://github.com/cablate/banini-tracker) to eliminate $11/mo in Apify + LLM API fees. Supports [skill-cron](skill-cron/) for scheduled runs |
